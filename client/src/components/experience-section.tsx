      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl lg:text-4xl font-bold text-navy-800 mb-4">
            <Briefcase className="inline mr-3 text-gold" />
            Work Experience
          </h2>
        ...
        <div className="space-y-8">
          {experiences.map((experience, index) => (
            <div key={index} className="bg-white rounded-xl p-8 shadow-lg border border-beige-200 hover:shadow-xl transition-shadow">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
                <div>
                  <h3 className="text-2xl font-bold text-navy-800 mb-2">{experience.title}</h3>
                  <p className="text-lg text-gold font-semibold mb-1">{experience.company}</p>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {experience.achievements.map((achievement, achievementIndex) => (
                  <div key={achievementIndex} className="flex items-start">
                    <achievement.icon className="text-gold mt-1 mr-3 h-5 w-5 flex-shrink-0" />
                    <p className="text-gray-700">
                      {achievement.text.split(/(\$50M MXN|\$50K\+ MXN|3,000\+|50\+|8 junior analysts|15 days|15\+|40%|5\+)/).map((part, partIndex) => {
                        if (/(\$50M MXN|\$50K\+ MXN|3,000\+|50\+|8 junior analysts|15 days|15\+|40%|5\+)/.test(part)) {
                          return <span key={partIndex} className="font-bold text-navy-800">{part}</span>;
                        }
                        return part;
                      })}
                    </p>
